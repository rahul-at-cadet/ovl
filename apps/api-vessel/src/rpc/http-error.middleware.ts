 ...
 * anything that is not a TRPCError becomes INTERNAL_SERVER_ERROR, so a
 * missing report, a failed validation and a section held by a colleague
 * all reached the client as a 500 with a stack trace attached.
 *
 ...
 * NotFoundException('...')` and an object for the structured form the
 * validation path uses (`{ message, errors }`). Both have to survive:
 * the field-level errors are what the report form renders against
 * individual inputs.
 */
