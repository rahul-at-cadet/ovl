...
function makeTrpc(pullConfig: (input: any) => any) {
  ...
  return {
    ...
    client: {
      ...
      // Schema catalogue pull. These specs are about sync-cycle *outcomes*, so
      // it answers "nothing changed" and the schema half stays out of the way.
      catalogue: {
        edge: {
          pullSchemas: { query: async () => ({ changed: [], removed: [] }) },
        },
      },
    },
  } as any;
...
// Constructor deps the tenancy work added between trpc and auth. Nothing here
// asserts on schema sync, so they only need to be inert.
const schemaSync = { known: async () => [], apply: async () => ({ applied: [], removed: [], rejected: [] }) } as any;
const schemaRegistry = { reload: async () => {} } as any;
...
describe('SyncService', () => {
  ...
  it('sends its own name and IMO, and flags a divergence from shore', async () => {
    ...
    const svc = new SyncService(trpc, schemaSync, schemaRegistry, auth, db);
