...
export class ComplianceService {
  ...
  async listProfiles() {
    const rows = await this.db.select().from(schema.regulatoryProfileAssignments);
    return rows.map((r) => ({
  ...
  async saveProfile(scope: Scope, profiles: string[]) {
    ...
    if (existing.length > 0) {
      ...
      await this.db.insert(schema.regulatoryProfileAssignments).values({
  ...
  async listCadenceRules() {
    const rows = await this.db.select().from(schema.cadenceRules);
    return rows.map((r) => ({
      scope: scopeFromRow(r),
  ...
  async saveCadenceRule(scope: Scope, minReportIntervalHours: number, maxGapHours: number) {
    ...
    if (existing.length > 0) {
      ...
      await this.db.insert(schema.cadenceRules).values({
        scopeType: scope.type,
        vesselId: scope.type === 'vessel' ? scope.key ?? null : null,
        groupTag: scope.type === 'group' ? scope.key ?? null : null,
  ...
  async listRuleSeverities() {
    const rows = await this.db.select().from(schema.ruleSeverityAssignments);
    return rows.map((r) => ({
      scope: scopeFromRow(r),
  ...
  async saveRuleSeverity(scope: Scope, severities: Record<string, string>) {
    ...
    if (existing.length > 0) {
      ...
      await this.db.insert(schema.ruleSeverityAssignments).values({
        scopeType: scope.type,
        vesselId: scope.type === 'vessel' ? scope.key ?? null : null,
        groupTag: scope.type === 'group' ? scope.key ?? null : null,
        severities,
